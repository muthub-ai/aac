...
import { Search, X, Puzzle, Plus, ChevronDown, ChevronUp } from 'lucide-react';
          ...
          <div className="mb-5 space-y-3">
            <div className="flex items-center gap-3">
              ...
              <button
                type="button"
                className={cn(
                  'inline-flex shrink-0 items-center gap-2 rounded-lg px-4 py-2.5 text-sm font-semibold text-white',
                  'bg-ring shadow-sm transition-all hover:bg-ring/90 hover:shadow-md',
                  'focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring/50 focus-visible:ring-offset-2',
                )}
                aria-label="Submit a new pattern"
              >
                <Plus className="h-4 w-4" strokeWidth={2.5} />
                <span className="hidden sm:inline">Submit Pattern</span>
              </button>
            </div>
            ...
            {allTags.length > 0 && (
              <div className="flex flex-wrap items-center gap-2">
                ...
                {(tagsExpanded ? allTags : allTags.slice(0, VISIBLE_TAG_COUNT)).map((tag) => (
                  <button
                    key={tag}
                ...
                {allTags.length > VISIBLE_TAG_COUNT && (
                  <button
                    ...
                    aria-label={tagsExpanded ? 'Show fewer tags' : `Show all ${allTags.length} tags`}
                  >
                    {tagsExpanded ? (
                      <>
                        Less <ChevronUp className="h-3 w-3" strokeWidth={2} />
                      </>
                    ) : (
                      <>
                        +{allTags.length - VISIBLE_TAG_COUNT} more <ChevronDown className="h-3 w-3" strokeWidth={2} />
                      </>
                    )}
                  </button>
                )}
