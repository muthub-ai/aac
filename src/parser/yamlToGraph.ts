...
export function yamlToGraph(yamlText: string): ParseResult {
  ...
  // Parse software systems and their containers/components
...
/**
 * Resolve a relationship reference to an existing node ID.
 * Handles shorthand (e.g., "EmailSystem") and dotted (e.g., "InternetBanking.WebApp").
 */
function resolveNodeId(ref: string, nodeIds: Set<string>): string | null {
  if (nodeIds.has(ref)) return ref;

  // Try matching as a suffix (e.g., "WebApp" matches "InternetBanking.WebApp")
